...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1291
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1848
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11291
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11848
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 1291
	%{_sbindir}/semanage port -d -p tcp 1848
	%{_sbindir}/semanage port -d -p tcp 11291
	%{_sbindir}/semanage port -d -p tcp 11848
	for selinuxvariant in %{selinux_variants}; do
